...
public:
   ...
   void operator()(ADvector& fg, const ADvector& vars)
   {
      ...
      // cost multipliers
      static const double MULT_C     =  200;
      static const double MULT_E     =  500;
      static const double MULT_V     =    1;
      static const double MULT_D     =  500;
      static const double MULT_A     =    1;
      static const double MULT_DDIFF = 9500;
      static const double MULT_ADIFF =    1; 

      // The part of the cost based on the reference state.
      for (int t = 0; t < mpc_.N; ++t)
      {
         fg[0] += MULT_C * CppAD::pow(vars[mpc_.Cidx + t], 2);
         fg[0] += MULT_E * CppAD::pow(vars[mpc_.Eidx + t], 2);
         fg[0] += MULT_V * CppAD::pow(vars[mpc_.Vidx + t] - mpc_.V_REFERENCE, 2);
         //fg[0] += MULT_DIFF * CppAD::pow(pow(vars[mpc_.Xidx + t] - xEnd, 2) + pow(vars[mpc_.Yidx + t] - yEnd, 2), 0.5);
         if (t < mpc_.N - 1)
         {
            fg[0] += MULT_D * CppAD::pow(vars[mpc_.Didx + t], 2);
            fg[0] += MULT_A * CppAD::pow(vars[mpc_.Aidx + t], 2);
            if (t < mpc_.N - 2)
            {
               fg[0] += MULT_DDIFF * CppAD::pow(vars[mpc_.Didx + t + 1] - vars[mpc_.Didx + t], 2);
               fg[0] += MULT_ADIFF * CppAD::pow(vars[mpc_.Aidx + t + 1] - vars[mpc_.Aidx + t], 2);
            }
         }
      }
...
MPC::MPC(
   const int n, const double& dt, const double& lat, const int order,
